...
private fun SourceSwitchPreference(
    ...
    TextPreferenceWidget(
        ...
        title = if (source.labelAsName) {
            source.animeSource.toString()
        } else {
            LocaleHelper.getSourceDisplayName(source.animeSource.lang, context)
        },
        widget = {
            Row(
                verticalAlignment = Alignment.CenterVertically,
            ) {
                if (source.animeSource is ConfigurableAnimeSource) {
                    IconButton(onClick = { onClickSourcePreferences(source.animeSource.id) }) {
                        Icon(
        ...
        onPreferenceClick = { onClickSource(source.animeSource.id) },
    )
}
