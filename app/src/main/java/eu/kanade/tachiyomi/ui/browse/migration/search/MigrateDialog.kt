...
internal class MigrateAnimeDialogScreenModel(
    ...
    private suspend fun migrateAnimeInternal(
        ...
        try {
            syncEpisodesWithSource.await(sourceEpisodes, newAnime, newAnimeSource)
        } catch (_: Exception) {
        ...
        // Delete downloaded
        if (deleteDownloaded) {
            if (oldAnimeSource != null) {
                downloadManager.deleteAnime(oldAnime, oldAnimeSource)
            }
        }
