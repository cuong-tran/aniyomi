...
class MyAnimeListApi(
    ...
    suspend fun updateItem(track: AnimeTrack): AnimeTrack {
        return withIOContext {
            val formBodyBuilder = FormBody.Builder()
                ...
                .add("is_rewatching", (track.status == MyList.REWATCHING).toString())
    ...
    suspend fun deleteAnimeItem(track: Track) {
        withIOContext {
            authClient
    ...
    private fun parseAnimeItem(listStatus: MALListAnimeItemStatus, track: AnimeTrack): AnimeTrack {
        return track.apply {
            ...
            status = if (isRewatching) MyList.REWATCHING else getStatus(listStatus.status)
