...
class DownloadCache(
    ...
    suspend fun removeSource(animeSource: AnimeSource) {
        rootDownloadsDirMutex.withLock {
            rootDownloadsDir.sourceDirs -= animeSource.id
        }
    ...
    private fun renewCache() {
        ...
        renewalJob = scope.launchIO {
            ...
            var animeSources = emptyList<AnimeSource>()
            withTimeoutOrNull(30.seconds) {
                extensionManager.isInitialized.first { it }
                sourceManager.isInitialized.first { it }

                animeSources = getSources()
            }

            val sourceMap = animeSources.associate {
