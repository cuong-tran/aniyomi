...
class DownloadManager(
     ...
     */
    fun buildVideo(animeSource: AnimeSource, anime: Anime, episode: Episode): Video {
        val episodeDir =
            provider.findEpisodeDir(episode.name, episode.scanlator, anime.title, animeSource)
     ...
     * @param anime the anime of the episodes.
     * @param animeSource the source of the episodes.
     */
    fun deleteEpisodes(episodes: List<Episode>, anime: Anime, animeSource: AnimeSource) {
        launchIO {
            ...
            if (animeDir?.listFiles()?.isEmpty() == true) {
                deleteAnime(anime, animeSource, removeQueued = false)
            }
        }
    }
     ...
     */
    fun deleteAnime(anime: Anime, animeSource: AnimeSource, removeQueued: Boolean = true) {
        launchIO {
            if (removeQueued) {
                downloader.removeFromQueue(anime)
            }
            provider.findAnimeDir(anime.title, animeSource)?.delete()
            cache.removeAnime(anime)
            // Delete source directory if empty
            val sourceDir = provider.findSourceDir(animeSource)
            if (sourceDir?.listFiles()?.isEmpty() == true) {
                sourceDir.delete()
                cache.removeSource(animeSource)
            }
        }
    }
     ...
     *
     * @param oldAnimeSource the old source.
     * @param newAnimeSource the new source.
     */
    fun renameSource(oldAnimeSource: AnimeSource, newAnimeSource: AnimeSource) {
        val oldFolder = provider.findSourceDir(oldAnimeSource) ?: return
        val newName = provider.getSourceDirName(newAnimeSource)
     ...
     */
    suspend fun renameEpisode(animeSource: AnimeSource, anime: Anime, oldEpisode: Episode, newEpisode: Episode) {
        val oldNames = provider.getValidEpisodeDirNames(oldEpisode.name, oldEpisode.scanlator)
        val animeDir = provider.getAnimeDir(anime.title, animeSource)
